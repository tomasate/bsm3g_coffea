...
def add_muon_boost_weight(events, weights, year, variation, dataset):
    if dataset.startswith("TTTo"):
        # get input values
        met_pt = events.selected_met.pt
        muons_pt = ak.firsts(events.selected_muons).pt
        ...
        # load correction set
        json_path = f"{Path.cwd()}/analysis/data/{year}_2b1mu_boost_weight.json.gz"
